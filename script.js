...
async function fetchData() {
    ...
    const API_BASE = 'https://1.darkeaglehome.workers.dev';
    let url = API_BASE + '/api/traffic';
...
async function exportCSV() {
    ...
    try {
        ...
        // CSV Header
        let csvContent = "Tarih,Trafik Indeksi\n";
        
        // CSV Rows
        data.forEach(item => {
            csvContent += `${item.traffic_index_date},${item.traffic_index}\n`;
        });
        ...
        const fileName = `trafik_verisi_${new Date().toISOString().slice(0,10)}.csv`;
        ...
        showToast('CSV başarıyla indirildi.');
    } catch (error) {
...
fetchData();
setInterval(fetchData, 3600000); // Auto-refresh data every hour
